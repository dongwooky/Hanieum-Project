...
class CameraDetection:
    ...
    def __init__(self):
        ...
        self.receipt_list = []
         ...
         self.x_ocr_secret = "ZlBWaUFqdFp4bVFZTUpnTHZucGVwZFVOZUt3cllVY0o="
         self.ocr_invoke_url = "https://f04b1d88f83e41a6b1df8ce399b61fb5.apigw.ntruss.com/custom/v1/9782/eed9df9be97433637c2950146e96ff0956759904a38b1e5e6a9a69a7f6691a68/general"
         self.uuid = "ce3e4c43-4c69-4188-ba2a-8ad24d9615e4"
         
         self.headers = {
             "X-OCR-SECRET" : self.x_ocr_secret,
    ...
    def camera_detect(self):
        ...
        while True:
            ...
            if is_rectangle == True:    #사각형이면 멈추기
                ...
                cv2.imwrite('./dst_frame.png', dst_frame)
                timestamp = round(time.time())
                ret, buffer = cv2.imencode('.png', dst_frame)   #이미지를 png형태로 변환
                png_as_text = base64.b64encode(buffer)  #이미지를 b64로 인코딩
    
                data = json.dumps(self.data) #파이썬 객체를 json로 변환
                response = requests.post(self.ocr_invoke_url, headers=self.headers, data=data)    #API로 보내기
                res = json.loads(response.text) #json을 객체로 변환
                
                res_array = res.get('images')
                file = open('./food_list.txt', 'w')
                food_label = open('./food_label.txt', 'r', encoding='utf-8')
